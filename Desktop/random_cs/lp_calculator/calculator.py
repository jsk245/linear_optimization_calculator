...
# all equations of form >=, <=, or =, (>= and <= have no space)
# assume maximization problem
# dont use = sign in var names
# when declaring a var, we only accept >=0 or nothing written
# use the exact format as in example (word + colon without space is mandatory)
# must use multiplication symbol
# dont use any math symbols in variable names (including '-')
# all integer variables are assumed to be strictly >= 0
# you must declare variables before listing constraints
# dont add negative numbers, just use subtraction
# dont use parantheses
# only include a single value on the right side of the constraint, and if it is negative, there must be no space bt the sign and num
@app.command()
